...
export function SunlightChart() {
  ...
  useEffect(() => {
    const getChartData = async () => {
      try {
        setLoading(true)
        const response = await fetchData<ApiResponse>("/api/sunlight/chart")
        setData(response.data)
        setError(null)
      } catch (err) {
        console.error("Error fetching sunlight chart data:", err)
  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      <LineChart data={data}>
        ...
        <Line type="monotone" dataKey="Zone C" stroke="#22c55e" strokeWidth={2} />
        <Line type="monotone" dataKey="Zone D" stroke="#06b6d4" strokeWidth={2} />
      </LineChart>
    </ResponsiveContainer>
  )
}
