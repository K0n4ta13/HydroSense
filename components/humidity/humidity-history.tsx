...
export function HumidityHistory() {
  ...
  useEffect(() => {
    const getHistoryData = async () => {
      try {
        setLoading(true)
        const response = await fetchData<ApiResponse>("/api/humidity/history")
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          <TableHead>Punto de Rocío</TableHead>
      ...
      <TableBody>
        {readings.map((reading) => (
          <TableRow key={`${reading.time}-${reading.zone}`}>
            ...
            <TableCell>{reading.dewPoint}</TableCell>
            <TableCell>
              <Badge variant={reading.status === "High" ? "destructive" : "secondary"}>
                {reading.status === "High" ? "Alta" : "Óptimo"}
              </Badge>
            </TableCell>
          </TableRow>
        ))}
